...
function HeaderPatientInfo({ servicesManager, appConfig }: withAppTypes) {
  ...
  return (
    ...
    >
      {isMixedPatients ? (
        <Icons.MultiplePatients className="text-highlight" />
      ) : (
        <Icons.Patient className="text-highlight" />
      )}
      <div className="flex flex-col justify-center">
        {expanded ? (
          ...
          <div className="text-highlight self-center text-[13px]">
            {isMixedPatients ? 'Multiple Patients' : 'Patient'}
          </div>
        )}
      </div>
      <Icons.ArrowLeft className={`text-highlight ${expanded ? 'rotate-180' : ''}`} />
    </div>
  );
}
