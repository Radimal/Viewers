 ...
 *
 * Every event captured through this helper carries `ms_since_navigation_start`,
 * so any two of them in a page load can be subtracted to get a leg duration.
 * PostHog's own events ($identify, $snapshot) do not pass through here and carry
 * no such property.
 *
 * That subtraction has to stay on ONE clock. `page_load_started_at` and
 * `$initialization_time` are client wall clocks; PostHog's `timestamp` is
 * server-corrected, so mixing them absorbs the reader's machine-clock skew
 * instead of measuring latency. Enough readers carry real skew for that to
 * dominate the tail rather than round out of it.
