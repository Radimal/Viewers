...
// Clear caches from previous CACHE_VERSIONs on activation. Workbox's
// precache caches are left alone — workbox handles its own revision
// cleanup, and deleting them here would throw away the assets that were
// just precached during install.
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys().then(cacheNames => {
      return Promise.all(
        cacheNames.map(cacheName => {
          const isOwnCache = cacheName.startsWith('static-') || cacheName.startsWith('fonts-');
          if (isOwnCache && !cacheName.endsWith(CACHE_VERSION)) {
